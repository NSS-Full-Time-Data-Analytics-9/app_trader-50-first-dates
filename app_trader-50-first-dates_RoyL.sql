...
-- START 
-- Get list of apps from both stores
SELECT name
FROM app_store_apps
INNER JOIN play_store_apps
		   USING (name);
-- Note: 553 apps are in both stores

-- Get ratings for these 553 apps from both stores
...
INNER JOIN play_store_apps AS psa 
		   USING (name)
GROUP BY name, asa_rating, psa_rating
ORDER BY avg_rating DESC, asa_rating DESC, psa_rating DESC;
-- Note: only 329 apps have ratings in both stores - these are ranked by average rating.

-- Next, look at app price to calculate purchase price.
SELECT DISTINCT name, 
	   asa.rating AS asa_rating, 
	   psa.rating AS psa_rating, 
	   (asa.rating + psa.rating)/2 AS avg_rating,
...
INNER JOIN play_store_apps AS psa 
		   USING (name)
WHERE asa.price <= 2.5
GROUP BY name, asa_rating, psa_rating, asa.price, psa.price, asa.review_count, psa.review_count
ORDER BY avg_rating DESC, asa_rating DESC, psa_rating DESC;
-- Returns 283 rows

-- Filter by avg_rating <= 4.5
SELECT DISTINCT name, 
	   asa.rating AS asa_rating, 
	   psa.rating AS psa_rating, 
	   (asa.rating + psa.rating)/2 AS avg_rating,
...
INNER JOIN play_store_apps AS psa 
		   USING (name)
WHERE asa.price <= 2.5 AND ((asa.rating + psa.rating)/2) >= 4.5
GROUP BY name, asa_rating, psa_rating, asa.price, psa.price, asa.review_count, psa.review_count
ORDER BY avg_rating DESC, asa_rating DESC, psa_rating DESC;
-- Returns 80 rows			  
						  
-- Keep apps regardless of purchase price
SELECT DISTINCT name, 
	   asa.rating AS asa_rating, 
	   psa.rating AS psa_rating, 
	   (asa.rating + psa.rating)/2 AS avg_rating,
...
-- Calculate revenue based on average of the 2 ratings rounded down to 0.25 as months based on the research.
-- Then multiplied by $4,000 ($5000 income from in-app advertising and purchases - $1000 for marketing) per month to show cumulative income.
-- Convert price from both tables and cum income to money format.
SELECT DISTINCT name, 
	   asa.rating AS asa_rating, 
	   psa.rating AS psa_rating, 
	   (asa.rating + psa.rating)/2 AS avg_rating,
	   ...
	   ((((FLOOR (((asa.rating + psa.rating)/2)/0.25)*0.25)/.25*6)+12) * 4000)::money AS cum_income	  	
FROM app_store_apps AS asa
INNER JOIN play_store_apps AS psa 
		   USING (name)
WHERE ((asa.rating + psa.rating)/2) >= 4.5
GROUP BY name, asa_rating, psa_rating, asa.price, psa.price, asa.review_count, psa.review_count
ORDER BY cum_income DESC, avg_rating DESC, asa_rating DESC, psa_rating DESC;

						  
-- Work In Progress: add new column for purchase price						  
CASE WHEN asa.price::money = psa.price::money AND asa.price::money = 0 THEN 25000
	 WHEN asa.price::money >= psa.price::money THEN asa.price::money * 10000						  
     ELSE psa.price::money * 10000 END AS purchase_price						  
